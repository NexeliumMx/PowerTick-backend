...
async function insertMeasurements() {
  try {
    // Prompt the user for the serial number, starting year, and ending year
    ...
    // Validate the inputs
    if (!serialNumber || isNaN(startYear) || isNaN(endYear) || startYear > endYear) {
      console.error("Invalid serial number or year input.");
      return;
    }

    // Variables to accumulate consumption across the entire period
