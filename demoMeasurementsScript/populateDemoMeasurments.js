// Description:
// This script populates the `demo.measurements` table in a PostgreSQL database
// with synthetic measurements for an energy meter over a specified time range.
// Measurements are generated every 5 minutes for each day in the time range.
//
// Objectives:
// - Simulate realistic energy meter data including voltage, current, power, and energy values.
// - Allow the user to specify the starting year and duration (in years).
// - Automatically compute all timestamps and generate data for every 5-minute interval.
//
// Conditions for Results:
// - The script generates measurements for each 5-minute interval for all days
//   in the specified year(s).
// - Data is randomized within realistic ranges for electrical parameters.
// - Measurements include cumulative energy calculations based on the interval's power readings.
...
async function insertMeasurements() {
  try {
    // Prompt the user for the serial number, starting year, and number of years
    const serialNumber = prompt("Enter the serial number (e.g., DEMO0000001): ");
    const yearInput = prompt("Enter the starting year (e.g., 2023): ");
    const numberOfYearsInput = prompt("Enter the number of years to write (e.g., 1 or 2): ");

    const startYear = parseInt(yearInput, 10);
    const numberOfYears = parseInt(numberOfYearsInput, 10);

    // Validate the inputs
    if (!serialNumber || isNaN(startYear) || isNaN(numberOfYears) || numberOfYears <= 0) {
      console.error("Invalid serial number, starting year, or number of years.");
      return;
    }
    ...
    for (let year = startYear; year <= endYear; year++) {
      console.log(`Writing measurements for the year: ${year}`);

      // Loop through each month in the year
      for (let month = 1; month <= 12; month++) {
        console.log(`Processing month: ${month} of ${year}`);
