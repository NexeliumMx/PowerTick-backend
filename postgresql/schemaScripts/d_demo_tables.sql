/**
 * FileName: postgresql/schemaScripts/d_demo_tables.sql
...
CREATE TABLE IF NOT EXISTS powermeters (
  ...
  reset_counters INT,
  reset_dmd_max INT,
  CONSTRAINT pk_demo_powermeters PRIMARY KEY (powermeter_id),
  CONSTRAINT uq_demo_powermeters_serial_number UNIQUE (serial_number),
  CONSTRAINT fk_demo_powermeters_client_id_clients FOREIGN KEY (client_id) REFERENCES public.clients(client_id) ON DELETE CASCADE,
  CONSTRAINT fk_demo_powermeters_installation_id_installations FOREIGN KEY (installation_id) REFERENCES public.installations(installation_id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS measurements (
  ...
  CONSTRAINT pk_demo_measurements PRIMARY KEY (powermeter_id, timestamp),
  CONSTRAINT fk_demo_measurements_powermeter_id_powermeters FOREIGN KEY (powermeter_id) REFERENCES powermeters(powermeter_id) ON DELETE CASCADE
);
...
DECLARE
    r RECORD;
BEGIN
    FOR r IN SELECT tablename FROM pg_tables WHERE schemaname = 'demo'
    LOOP
        EXECUTE 'ALTER TABLE demo.' || quote_ident(r.tablename) || ' OWNER TO azure_pg_admin;';
    END LOOP;
END $$;