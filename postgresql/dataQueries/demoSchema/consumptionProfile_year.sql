 ...
 *
 * 4. Consumption Calculation: For each month, the query calculates the difference between the oldest and newest measurements 
...
WITH monthly_data AS (
  SELECT
    ...
    row_number() OVER (PARTITION BY p.time_zone, date_trunc('month', m."timestamp" AT TIME ZONE p.time_zone) ORDER BY m."timestamp" AT TIME ZONE p.time_zone ASC) AS row_asc,
    row_number() OVER (PARTITION BY p.time_zone, date_trunc('month', m."timestamp" AT TIME ZONE p.time_zone) ORDER BY m."timestamp" AT TIME ZONE p.time_zone DESC) AS row_desc
